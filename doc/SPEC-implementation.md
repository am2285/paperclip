...
- Structured JSON work products use `type: "structured_output"` and metadata with `contractVersion`, `runKey`, `sourceSnapshotHash`, nested JSON `result`, and `resultHash`. They are separate from attachment-backed artifacts and can be primary `ready_for_review` outputs.
