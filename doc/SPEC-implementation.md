...
- Structured JSON work products use `type: "structured_output"` and metadata with `contractVersion`, `runKey`, `sourceSnapshotHash`, nested JSON `result`, and `resultHash`. Hash-bound reviewer products may additionally carry root `reviewer`, `reviewedResultHash`, and `reviewedSourceWorkProductId` and/or `sourceWorkProductId`; reviewer lineage is all-or-none and both source ids must agree when present. They are separate from attachment-backed artifacts and can be primary `ready_for_review` outputs.
